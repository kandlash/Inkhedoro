...
func _process(delta):
	...
	# Инверсия, чтобы камера тянулась за курсором
	...
	var target_ui_offset := Vector2(nx, ny) * sway_amount
	var final_pos := arm_base_start_pos + target_ui_offset
	arm_base.position = arm_base.position.lerp(final_pos, delta * 2.0)
	tattoo_pose.position = tattoo_pose.position.lerp(final_pos, delta * 2.0)
